...
echo -e "scaffold\tfragment\tprophage_mean_coverage\tprophage_median_coverage\thost_mean_coverage\thost_median_coverage\tmean_coverage_ratio\tmedian_coverage_ratio" > "$OUTPUT_FILE"
...
tail -n +2 "$COORD_FILE" | while IFS=$'\t' read -r scaffold fragment start stop; do
    ...
    # Calculate mean and median coverage for non-prophage (host) regions
    non_prophage_means=()
    non_prophage_medians=()
    ...
    while IFS=$'\t' read -r np_start np_end; do
        if [ "$np_start" -le "$np_end" ]; then
            ...
            non_prophage_means+=("$np_mean")
            non_prophage_medians+=("$np_median")
        fi
    done < "$tmp_non_prophage"

    rm -f "$tmp_non_prophage"

    # Calculate overall mean and median coverage for non-prophage regions
    non_prophage_mean=$(IFS=+; bc <<< "${non_prophage_means[*]}")
    non_prophage_mean=$(awk "BEGIN {print $non_prophage_mean / ${#non_prophage_means[@]}}")
    
    non_prophage_median=$(IFS=+; bc <<< "${non_prophage_medians[*]}")
    non_prophage_median=$(awk "BEGIN {print $non_prophage_median / ${#non_prophage_medians[@]}}")
    ...
    echo "Mean coverage ratio (prophage/host): $mean_coverage_ratio"
    echo "Median coverage ratio (prophage/host): $median_coverage_ratio"

    # Write the results to the output file
    echo -e "${scaffold}\t${fragment}\t${prophage_mean}\t${prophage_median}\t${non_prophage_mean}\t${non_prophage_median}\t${mean_coverage_ratio}\t${median_coverage_ratio}" >> "$OUTPUT_FILE"
